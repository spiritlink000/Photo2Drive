...
/// View displaying upload progress and status for cloud storage.
struct CloudUploadView: View {
    @Bindable var viewModel: CloudUploadViewModel
    ...
    var body: some View {
        ...
        .task {
            await viewModel.startUpload(storageManager: storageManager)
        }
    }
        ...
        VStack(spacing: 12) {
            if viewModel.isCompleted {
                ...
                if let folder = viewModel.selectedFolder {
                    Text("Uploaded to \(folder.storageType.displayName)")
                        .font(.subheadline)
    ...
    private func uploadItemRow(_ item: CloudUploadItem) -> some View {
        HStack {
            // サムネイル
    ...
    private var actionButtons: some View {
        VStack(spacing: 12) {
            ...
            if viewModel.failedCount > 0 && !viewModel.isUploading {
                Button {
                    Task {
                        await viewModel.retryFailed(storageManager: storageManager)
                    }
                } label: {
                    Text("Retry Failed")
                        .frame(maxWidth: .infinity)
